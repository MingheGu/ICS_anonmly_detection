...
def refine_packet_label(
    ...
    attacker_ip: str,
    target_ip: str,
    internal_ip: str = "192.168.95.5",
) -> str:
    """
    Refine a coarse time-window label into a per-packet label.

    Time window says "this period is attack_X", but most packets in that window
    are still normal traffic (HMI polling, PLC responses, etc). This function
    checks each packet's source IP and Modbus FC to decide if it's actually
    an attack packet.
    """
    ...
    fc = features.get("function_code", -1)
    dst_port = features.get("dst_port", 0)
    protocol = features.get("protocol", "")
    tcp_flags = str(features.get("tcp_flags", ""))
    payload_len = int(features.get("payload_len", 0))

    # External attack: write injection (Attack A: fc=5, B-external: fc=6)
    if coarse_label == "attack_inject":
        if src_ip == attacker_ip and dst_port == 502 and fc in {5, 6}:
            return "attack_inject"
        return "normal"

    # Internal attack: high-frequency write from compromised EWS (Attack B-internal)
    if coarse_label == "attack_inject_internal":
        if src_ip == internal_ip and dst_port == 502 and fc == 6:
            return "attack_inject_internal"
        return "normal"

    # External slow read attack (Attack C-external)
    if coarse_label == "attack_slow_external":
        if src_ip == attacker_ip and dst_port == 502 and fc == 3:
            return "attack_slow_external"
        return "normal"

    # Internal slow read attack (Attack C-internal)
    if coarse_label == "attack_slow_internal":
        if src_ip == internal_ip and dst_port == 502 and fc == 3:
            return "attack_slow_internal"
        return "normal"

    # Internal stealth write attack (Attack D-internal): same network signature
    # as legitimate EWS writes, distinguishable only by timing window
    if coarse_label == "attack_stealth_write_internal":
        if src_ip == internal_ip and dst_port == 502 and fc == 6:
            return "attack_stealth_write_internal"
        return "normal"

    # Scan attack (Attack E: nmap from Kali)
    if coarse_label == "attack_scan":
        ...
        if protocol == "TCP":
            ...
            is_non_modbus_port = dst_port != 502
            if is_syn_probe or is_non_modbus_port:
                return "attack_scan"
        return "normal"
...
def process_pcap(
    ...
    with PcapReader(str(pcap_path)) as reader:
        for packet in reader:
            ...
            label = refine_packet_label(coarse_label, features, attacker_ip, target_ip, internal_ip)
...
def assign_split_session(
    ...
    if row["pcap_name"] in {"mixed_long_03", "mixed_long_04"}:
        return "test"
    if row["pcap_name"] == session_pcap_name and session_attack_onset_s is not None:
...
def assign_packet_split(
    ...
    session_pcap_name: str = "session_plc423",
) -> str:
    if row["pcap_name"] == "normal_long_00":
        return "train"
    if row["pcap_name"] == "normal_long_03":
...
def main() -> None:
    ...
    parser.add_argument(
        "--session-pcap-name",
        default="session_plc423",
        help="pcap_name that should use the session-attack-onset based split logic.",
    )
    parser.add_argument(
    ...
    internal_ip = config.get("internal_ip", "192.168.95.5")
    ...
    session_pcap_name = args.session_pcap_name
    packet_df["split"] = packet_df.apply(
        lambda row: assign_packet_split(row, onset, frac, session_pcap_name),
        axis=1,
    )
    ...
    window_df["split"] = window_df.apply(
        lambda row: assign_split_session(row, onset, frac, session_pcap_name),
        axis=1,
    )
