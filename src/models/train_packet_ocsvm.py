...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Drop TCP packets without Modbus payload (function_code=-1, "
             "i.e. SYN/ACK/FIN handshake noise). These dominate the dataset "
             "(~92%%) and dilute Modbus-layer signal.",
    )
    ...
    if args.exclude_tcp_fc_minus1:
        before = len(df)
        df = df[~((df["protocol"] == "TCP") & (df["function_code"].astype(int) == -1))].copy()
        print(f"Filtered TCP fc=-1: {before} -> {len(df)} packets ({100*(before-len(df))/before:.1f}% dropped)")
