...
def main() -> None:
    ...
    val_scores = anomaly_scores(model, val_contexts, val_targets, device)
    holdout_scores = anomaly_scores(model, holdout_contexts, holdout_targets, device) if len(holdout_contexts) else np.array([])
    test_scores = anomaly_scores(model, test_contexts, test_targets, device)
