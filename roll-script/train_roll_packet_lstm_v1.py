...
class PacketRollLSTM(nn.Module):
    def __init__(self, vocab_size: int, embed_dim: int = 8, hidden_size: int = 16) -> None:
        super().__init__()
...
def anomaly_scores(model: nn.Module, contexts: np.ndarray, targets: np.ndarray, device: torch.device) -> np.ndarray:
    model.eval()
    with torch.no_grad():
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="artifacts/rolling_fc_address/packet_results",
    ...
    plt.plot(val_losses, label="val_anomaly_score")
    plt.xlabel("Epoch")
